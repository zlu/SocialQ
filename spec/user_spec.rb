...
describe SocialContactCenter::User::Topsy do
  it "should return a Ruby hash of author info" do
    SocialContactCenter::User::Topsy::author_info('barackobama')['name'].should == 'Barack Obama'
  end
end

describe SocialContactCenter::User do
  before(:all) do
    @agent = SocialContactCenter::User.new({ :name         => 'Barack Obama', 
  ...
  it "should raise argument errors if a new user object is created without an option set" do
    begin
      result = SocialContactCenter::User.new({ :foo => :bar })
    rescue => e
      e.to_s.should == 'A hash with the :name set is required.'
    end
    
     begin
        result = SocialContactCenter::User.new({ :name => 'John Doe' })
      rescue => e
        e.to_s.should == 'A hash with the :twitter_user set is required.'
      end
      
    begin
      result = SocialContactCenter::User.new({ :name => 'John Doe', :twitter_user => 'johndoe' })
    rescue => e
      e.to_s.should == 'A hash with the :phone_number set is required.'
    end
    
    begin
      result = SocialContactCenter::User.new({ :name => 'John Doe', :twitter_user => 'johndoe', :phone_number => '+14155551212' })
    rescue => e
